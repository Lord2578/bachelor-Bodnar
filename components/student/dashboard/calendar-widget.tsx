    ...
    const fetchSchedule = async () => {
      try {
        ...
        if (userData.user.role === 'student') {
          const studentResponse = await fetch(`/api/students?userId=${userData.user.id}`)
          const studentData = await studentResponse.json()
          
          if (studentData.students && studentData.students.length > 0) {
            const studentId = studentData.students[0].id
            const scheduleResponse = await fetch(`/api/schedule?studentId=${studentId}`)
            ...
            if (scheduleData.classes && Array.isArray(scheduleData.classes)) {
              const calendarEvents = scheduleData.classes.map((cls: ScheduleClass) => ({
                id: cls.id,
                title: cls.description || `Заняття з ${cls.teacher.name}`,
    ...
    <div className="space-y-4 bg-white rounded-lg p-4 shadow-sm h-full flex flex-col">
      <div className="flex items-center justify-between">
        ...
        <Button variant="link" className="text-sm text-blue-500 shrink-0" asChild>
          <a href="/student/calendar">Переглянути все</a>
        </Button>
      </div>
